...
import com.teambr.bookshelf.common.blocks.{ConnectedTexturesState, BlockConnectedTextures}
...
import net.minecraftforge.client.model.{ISmartBlockModel, ISmartItemModel, TRSRTransformation}
...
class ModelConnectedTextures extends ISmartBlockModel with ISmartItemModel {
    ...
    def drawFace(connections : Array[Boolean], list : java.util.List[BakedQuad], rot : ModelRotation, facing : EnumFacing) {
        ...
        list.add(faceBakery.makeBakedQuad(new Vector3f(0.0F, 0.0F, 16.0F), new Vector3f(8.0F, 8.0F, 16.0F), face, block.getConnectedTextures.getTextureForCorner(2, connections), EnumFacing.SOUTH, rot, null, scale, true))
        list.add(faceBakery.makeBakedQuad(new Vector3f(8.0F, 0.0F, 16.0F), new Vector3f(16.0F, 8.0F, 16.0F), face, block.getConnectedTextures.getTextureForCorner(3, connections), EnumFacing.SOUTH, rot, null, scale, true))
        ...
        if (block != null && block.isTranslucent && world != null) {
            ...
            list.add(faceBakery.makeBakedQuad(new Vector3f(8.0F, 8.0F, 16.001F), new Vector3f(16.001F, 16.001F, 16.001F), face, block.getConnectedTextures.getTextureForCorner(1, connections), EnumFacing.NORTH, rot, null, scale, true))
        }
    }
